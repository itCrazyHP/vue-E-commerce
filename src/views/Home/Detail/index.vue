        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(spuSaleAttr) in goodInfo.spuSaleAttrList" :key="spuSaleAttr.id">
            ...
            <div class="cartWrap">
              <div class="controls">
                <input type="number" :value="skuNum" autocomplete="off" class="itxt" @change="skuNumHandler($event)">
...
<script lang="ts" setup>
  ...
  import {reqAddOrUpdataShopCart} from '@/api/reqApi'
  ...
  await home.getGoodInfo(route.params.skuId as string)
  
  const {goodInfo} = homeStore()
  const changeActive = (SaleAttrValue:any,SaleAttr:any)=>{
    SaleAttr.forEach((item: { isChecked: string }) => {
      item.isChecked = '0'
    })
    SaleAttrValue.isChecked='1'
  }

  const skuNumHandler = (event:Event)=>{
    skuNum.value=Number((<HTMLInputElement>event.target).value)
  }

  const addCart = async ()=>{
    if(store.get(cacheEnum.TOKEN_NAME)){
      let result =await reqAddOrUpdataShopCart(String(route.params.skuId),String(skuNum.value))
      if(result.code === 200){
      utils.sessionStore.set(cacheEnum.ADD_CART,home.goodInfo.skuInfo)
